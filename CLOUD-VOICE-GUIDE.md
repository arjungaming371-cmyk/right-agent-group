# Cloud Voice Guide — Priya's 100% cloud voice pipeline on AWS

The voice pipeline is **fully cloud** — the self-hosted Python services (local
Whisper + Edge TTS, what the demo originally ran) were REMOVED from the codebase
(`server/stt-service`, `server/tts-service` no longer exist). An AWS deployment
needs **no GPU, no Python, no venvs, no model downloads** — just Node 22,
PostgreSQL, ffmpeg, and API keys.
...
|---|---|---|
| STT (caller audio → text) | **Sarvam Saaras** (`saaras:v4`) — the ONLY STT | always on; `SARVAM_STT_MODEL` / `SARVAM_STT_MODE` tune it |
| TTS (reply text → voice) | **Sarvam Bulbul v3** (`bulbul:v3`, speaker `priya`) — default | `TTS_CALL_PROVIDER=sarvam` |
| TTS alternative | **Cartesia Sonic** (`sonic-3.6`) — very high quality | `TTS_CALL_PROVIDER=cartesia` |
| LLM (Priya's brain) | Groq (default) or **Sarvam-105B-conversations** | `LLM_PROVIDER=sarvam` |

One Sarvam key (`SARVAM_API_KEY`) covers STT + TTS + LLM and is **required** —
the voicebot refuses to boot without it. Cartesia needs its own key plus a
voice ID picked from <https://play.cartesia.ai/voices>.

**Why Sarvam STT is the right fit:** the conversation layer — LLM language
rules, WhatsApp memory, Lead Brain — is Roman-script based (Tenglish/
Hinglish). Sarvam's Saaras produces that natively: `mode=translit`
...
24kHz — which lands just as quiet on a phone line without normalization. The
...
2. Fill `.env` (every var is documented in `.env.example`):
   - `SARVAM_API_KEY` — REQUIRED (STT + default TTS share it). Sarvam STT
     defaults: model `saaras:v4`, mode `translit` — check
     <https://docs.sarvam.ai> for the current model tag and override
     `SARVAM_STT_MODEL` if a newer one ships.
...
3. There are no Python services to skip — the folders no longer exist.
   `START.ps1` starts only the website + voicebot + tunnel; on Linux just run
   the two processes under pm2.
...
The voicebot fails fast at boot if `SARVAM_API_KEY` is missing — you get
`FATAL: SARVAM_API_KEY is not set — ...` instead of a dead call at 9pm.
...
| Python 3.11 + two venvs (stt-service, tts-service) | Not needed — the services were removed from the repo |
