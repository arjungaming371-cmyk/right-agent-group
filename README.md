...
3. Copy `.env.example` → `.env` and fill every ❌ value (`GROQ_API_KEY` and Google login keys are REQUIRED; `WHATSAPP_APP_SECRET` is REQUIRED — the webhook is publicly abusable without it)
...
7. `npm run build`
8. `PowerShell -ExecutionPolicy Bypass -File START.ps1`  ← starts everything (Postgres check, Groq key check, STT, voicebot, website, tunnel)
...
- **AI Brain**: Groq Cloud API (Llama 3.3 70B, streaming) — first token in ~300-700ms, free tier
