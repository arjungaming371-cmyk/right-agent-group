...
# Voice provider check (read straight from .env — PowerShell child
# processes don't get dotenv). The voice pipeline is 100% cloud: Sarvam STT
# always, TTS via Sarvam (default) or Cartesia. No Python services exist.
...
if (-not $ttsCallProvider) { $ttsCallProvider = "sarvam" }
if (-not $llmProvider) { $llmProvider = "groq" }
if (-not (Read-EnvValue "SARVAM_API_KEY")) {
    Write-Host "      WARN SARVAM_API_KEY missing in .env - cloud STT + default TTS will fail! Get a key at dashboard.sarvam.ai" -ForegroundColor Red
}
...
if (-not $env:WHATSAPP_SERVICE_KEY) { Write-Host "      WARN WHATSAPP_SERVICE_KEY missing in .env - the voicebot cannot authenticate to the app" -ForegroundColor Red }
...
# 3. Voice pipeline: 100% cloud (Sarvam STT always; TTS = Sarvam | Cartesia).
...
Write-Host "  Voicebot  : ws://localhost:3002  |  STT: sarvam (cloud)  |  TTS: $ttsCallProvider (cloud)" -ForegroundColor White
